...
from mpc_control_fuzzy import MPCTrackingControlFuzzy
...
if __name__ == "__main__":
    dt_to = 0.1
    dt = 0.05
    horizon = 40  # shorter horizon to ease solves
    params = {"M": 0.15, 
              "L1": 7.05, "L2": 12.45, 
              "W1": 3.05, "W2": 2.95,
              "dt": dt,
    ...
    input_bound = {"lb": ca.DM([-5, -ca.pi/2]),
                   "ub": ca.DM([ 5,  ca.pi/2])}
    controller = MPCTrackingControlFuzzy(model, params,
                                         Q, R, 
    ...
    while time <= T_sim:
        ...
        if states is None or inputs is None:
            ...
            if failure_count <= 5 or failure_count % 20 == 0:
                print("MPC failed, reusing last control input.")
            u_con = last_control
            if consecutive_failures > 15:
                u_con = np.zeros(model.num_input)
            if consecutive_failures > 30:
                print("Too many consecutive MPC failures, stopping simulation.")
                break
        else:
            u_con = inputs[:,0]
            last_control = u_con
