...
def build_obstacles(open_spot: int, depth: float = 20.0):
    ...
    # Add left/right "wall" obstacles to keep the vehicle inside the aisle
    wall_width = 30.0
    span_end = x_start + 10 * (stall_width + stripe_width) - stripe_width  # end after last stall
    walls = [
        {
            "FL": {"X": -wall_width, "Y": depth},
            "FR": {"X": 0.0, "Y": depth},
            "BL": {"X": -wall_width, "Y": 0.0},
            "BR": {"X": 0.0, "Y": 0.0},
        },
        {
            "FL": {"X": span_end, "Y": depth},
            "FR": {"X": span_end + wall_width, "Y": depth},
            "BL": {"X": span_end, "Y": 0.0},
            "BR": {"X": span_end + wall_width, "Y": 0.0},
        },
    ]
...
def main():
    ...
    parser.add_argument(
        ...
        help="Test case name whose goal will be moved to the open stall center.",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    goal_y = 12.45  # park flush at trailer length

    # Always update test_cases.json for the chosen case
    ...
    # Always update initialize.json goal
